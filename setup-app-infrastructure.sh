...
for i in $(seq 1 20); do
  STATE=$(az containerapp env show --name $CONTAINER_ENV --resource-group $APP_RESOURCE_GROUP --query "properties.provisioningState" -o tsv 2>/dev/null || echo "Creating")
  echo "  Status: $STATE"
  [ "$STATE" = "Succeeded" ] && break
  [ "$STATE" = "Failed" ] && echo "Environment creation failed." && exit 1
  sleep 15
done
